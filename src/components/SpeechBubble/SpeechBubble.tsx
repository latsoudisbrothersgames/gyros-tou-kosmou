...
export function SpeechBubble({ lines, audible = true, voiceIso2 }: { lines: string[]; audible?: boolean; voiceIso2?: string }) {
  ...
  useEffect(() => {
    ...
    const blips = audible ? setInterval(() => {
      characters += 3;
      if (characters <= text.length) playSound('blip', voiceIso2);
